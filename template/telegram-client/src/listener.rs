...
pub struct Listener {
  ...
  exception: Option<Arc<Fn((&Api, &TGException)) -> TGResult<()> + Send + Sync + 'static>>,
...
impl Listener {
  ...
  pub fn on_exception<F>(&mut self, fnc: F) -> &mut Self where F: Fn((&Api, &TGException)) -> TGResult<()> + Send + Sync + 'static {
...
{% endif %}{% endfor %}
  ...
  /// when telegram client throw exception
  pub fn exception(&self) -> &Option<Arc<Fn((&Api, &TGException)) -> TGResult<()> + Send + Sync + 'static>> {
